...
class ConsumerImpl implements Consumer {
    ...
    private List<Event> events = Collections.synchronizedList(new ArrayList<>()); // Usar lista sincronizada se acessada
    ...
    public ConsumerImpl(String server, String password) {
        ...
        // Recomendo não usar Integer.MAX_VALUE diretamente para todos, pode ter
        // implicações.
        ...
        props.put(ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG, 500); // Tempo que o broker espera para atingir
                                                                 // FETCH_MIN_BYTES

        props.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, 300000); // 5 minutos
        props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, 30000); // 30 segundos (Padrão, mas pode aumentar se
                                                                    // necessário)
            ...
            consumerThread = new Thread(() -> {
                try {
                    ...
                    while (running.get()) {
                        ...
                        ConsumerRecords<String, byte[]> records = consumer.poll(Duration.ofSeconds(1)); // 1 segundo de
                                                                                                        // timeout
                        ...
                        if (!records.isEmpty()) { // Processar apenas se houver registros
                            ...
                            try {
                                consumer.commitSync();
                                System.out.println("Offsets commitados sincronamente após processar " + records.count()
                                        + " registros.");
                                ...
                                // Considere parar ou tratar a falha de commit, pois as mensagens podem ser
                                // reprocessadas.
                    ...
                    if (running.get()) { // Se não era uma parada intencional
                        System.err.println(
                                "WakeupException recebida, mas o consumidor não estava parando: " + e.getMessage());
